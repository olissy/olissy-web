...
@Component({
  ...
  templateUrl: './client-user-change-registration.component.html',
  styleUrls: ['./client-user-change-registration.component.css']
})

export class ClientUserChangeRegistrationComponent implements OnInit, OnDestroy {
  ...
  constructor(private authService:AuthService,
              private clienteUsuarioService:ClientUserChangeRegistrationService) { }
  ...
  public setIdDocInformularioCliente(FOREIGN_KEY){
    this.clienteUsuarioService.getIdDocByFOREIGN_KEY("client", FOREIGN_KEY).get().pipe(takeUntil(this.unsubscribe$)).subscribe( (res)=> {
      let PRIMARY_KEY = ""
  ...
  public alterarCadastroCliente(){
    ...
    if(this.formularioCliente.status === "VALID"){
      ...
      if(this.formularioCliente.get('imageNew').value === null){
        this.clienteUsuarioService.alterarDadosCliente(this.formularioCliente.get('PRIMARY_KEY').value, this.formularioCliente.value).then(()=>{
          this.loading = false
          this.markAsUntouched()
        })
      }else{
        this.clienteUsuarioService.setImagemStorage(`user/${this.formularioCliente.get('PRIMARY_KEY').value}.jpg`, this.formularioCliente.get('imageNew').value).then((url:any)=>{
          this.formularioCliente.patchValue({
            clientImageUrl : url,
